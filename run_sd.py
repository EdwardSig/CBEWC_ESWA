...
if __name__ == '__main__':
    nb_hidden = 64
    a = pd.read_csv('dataset/SD/new_ratings.csv')
    setup_seed(4)
    nb_user = a["userId"].max()
    nb_item = a["movieId"].max()
    train_set_file = 'dataset/SD/p_matrix.csv'
    test_set_file = 'dataset/SD/test.csv'
    prefer_weight_file = 'dataset/SD/prefer_matrix.npy'
    ...
    train(nb_user, nb_item, nb_hidden, epoches=200,
          train_dataloader=train_dataloader, w_dataloader=w_dataloader,
          lr=0.0015, nb_mask=5000,
