...
if __name__ == '__main__':
    nb_hidden = 128
    a = pd.read_csv('dataset/WD100/new_train_set.csv')
    b = a[['userId', 'movieId', 'rating', 'timestamp']]
    a = b
    setup_seed(20230501)
    nb_user = a["userId"].nunique()
    nb_item = a["movieId"].nunique()
    train_set_file = 'dataset/WD100/p_matrix.csv'
    test_set_file = 'dataset/WD100/test.csv'
    prefer_weight_file = 'dataset/WD100/prefer_matrix.npy'
    ...
    print('start train')
    train(nb_user, nb_item, nb_hidden, epoches=150, train_dataloader=train_dataloader, w_dataloader=w_dataloader,
